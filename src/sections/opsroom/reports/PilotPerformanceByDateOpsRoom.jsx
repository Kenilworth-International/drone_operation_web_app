...
const PilotPerformanceByDateOpsRoom = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const result = await dispatch(baseApi.endpoints.getPilotTeamSprayArea.initiate({
  ...
  const { headers, teamRows, totalsRow, grandTotal, dateKeys, monthLabel } = useMemo(() => {
    ...
    apiData.dates.forEach(day => {
      ...
      (day.pilots || []).forEach(p => {
        ...
        const value = Number(p.ops_room_dji_field_area || 0);
  ...
  const exportToExcel = () => {
    ...
    const filename = `Pilot_Performance_${formatDate(startDate)}_to_${formatDate(endDate)}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    const filename = `Pilot_Performance_${formatDate(startDate)}_to_${formatDate(endDate)}.pdf`;
    doc.save(filename);
  };
...
export default PilotPerformanceByDateOpsRoom;
