...
export default function HrSupportLogin() {
  const { isAuthenticated, login, checkEligibility, requestOtp, verifyOtp } = useHrSupportAuth();
  ...
  const handleSendOtp = async (e) => {
    e.preventDefault();
    if (phone.length !== 9) { setError('Enter your 9-digit mobile number'); return; }
    setLoading(true);
    setError('');
    try {
  ...
  return (
    <div className="hrsup-login-page">
      ...
      <div className="hrsup-login-sheet">
        ...
        {step === 'phone' ? (
          <form onSubmit={handleSendOtp} className="hrsup-login-form">
            <h2 className="hrsup-login-heading">Sign in</h2>
            <p className="hrsup-login-hint">Enter your registered mobile number to receive an OTP.</p>
